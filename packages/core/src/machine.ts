      ...
      },
      {
        actions: {
          ...
          connect: assign(context => {
            ...
            stopRecordFn?.();
            // Clear pending records but keep cursor so new IDs continue
            // incrementing. A full reset (cursor=0) would cause existing
            // mirrors to reject records whose id <= their cursor.
            buffer.buffer = {};
            // start a session
